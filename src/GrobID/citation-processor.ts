...
export interface CitationData {
  ...
  documentAuthors: Array<{firstName?: string, middleName?: string, lastName?: string, rawName?: string}>;
...
export class CitationProcessor {
  ...
  processTeiXml(teiXml: string, documentTitle = ''): CitationData {
    ...
    const documentAuthors = parser.extractDocumentAuthors(); 
  ...
  matchCitationsToReferences(citationData: CitationData): ReferenceUsage[] {
    ...
    // Remove any reference usages that don't have any contexts (unused references)
    // Comment this out if you want to keep unused references
    // return referenceUsage.filter(ru => ru.usageContexts.length > 0);
...
async function main() {
  if (process.argv.length < 3) {
    console.log('Usage: ts-node citation-processor.ts <pdf-path> [output-path]');
    process.exit(1);
  }
  ...
  try {
    ...
    // Output the data in JSON and CSV formats
