...
export class GrobidClient {
  ...
  /**
   * Get JSON annotations for references with coordinates in the PDF
   * @param pdfPath Path to the PDF file
   * @param options Processing options
   * @returns JSON with reference annotations
   */
  async getReferenceAnnotations(
    pdfPath: string,
    options: {
      consolidateCitations?: '0' | '1' | '2'
    } = {}
    ...
    try {
      const response = await axios.post(
        `${this.baseUrl}/api/referenceAnnotations`,
        formData,
        {
          headers: {
            ...formData.getHeaders(),
